...
\definecolor{mypink}{RGB}{219, 48, 122}\usepackage[ruled,vlined,figure]{algorithm2e}
...
{[}\textbf{tbl?}{]} is included in the input file; compare the placement
of the table here with the table in the printed output of this document.

\begin{table}

\caption{\label{tbl-freq}Frequency of Special Characters}
...
following this sentence is the point at which {[}\textbf{tbl?}{]} is
...
\caption{\label{tbl-commands}Some Typical Commands}
...
{[}\textbf{eq?}{]}, shown as an inline equation above:
...
and follow it with {[}\textbf{eq?}{]}, another numbered equation:
...
{[}\textbf{Editor00a?}{]} (so Editor00a's series should NOT be present
since it has no vol.~no.), a chapter in a divisible book
...
{[}\textbf{Clarkson85?}{]}, a master's thesis: {[}\textbf{anisi03?}{]},
an online document / world wide web resource {[}\textbf{Poker06?}{]}, a
video game (Case 1) {[}\textbf{Obama08?}{]} and (Case
2){[}\textbf{Novak03?}{]} and {[}\textbf{Lee05?}{]} and (Case 3) a
patent {[}\textbf{JoeScientist001?}{]}, work accepted for publication
{[}\textbf{rous08?}{]}, `YYYYb'-test for prolific author
