...
\newcommand{\fallback}[1]{{\fallbackFont #1}}\definecolor{mypink}{RGB}{219, 48, 122}\usepackage[vlined,figure]{algorithm2e}
\usepackage{tablefootnote}\makeatletter\@ifpackageloaded{caption}{}{\usepackage{caption}}\AtBeginDocument{%
...
\acmConference[Conference acronym \emoji{’}XX]{Make sure to enter the
correct conference title from your rights confirmation emai}{June
03\emoji{–}05, 2018}{Woodstock, NY}
\acmPrice{15.00}
...
ACM\emoji{’}s consolidated article template, introduced in 2017,
...
for any ACM publication \emoji{—} conference or journal, and for any
stage of publication, from review to final ``camera-ready'' copy, to the
author\emoji{’}s own version, with \emph{very} few changes to the
source.

\section{Some Algo}\label{some-algo}
...
be used to prepare many different kinds of documentation \emoji{—} a
double-blind initial submission of a full-length technical paper, a
two-page SIGGRAPH Emerging Technologies abstract, a ``camera-ready''
journal article, a SIGCHI Extended Abstract, and more \emoji{—} all by
...
further information, the \emph{\LaTeX User\emoji{’}s Guide} is available
from \url{https://www.acm.org/publications/proceedings-template}.
...
these parameters can be found in the \emph{\LaTeX User\emoji{’}s Guide.}
...
\item
  ...
  submission\emoji{’}s unique ID on each page of the work.
\item
...
Modifying the template \emoji{—} including but not limited to: adjusting
margins, typeface sizes, line spacing, paragraph and list definitions,
and the use of the \texttt{\textbackslash{}vspace} command to manually
adjust the vertical spacing between elements of your work \emoji{—} is
not allowed.
...
affiliation. Authors\emoji{’} names should not be abbreviated; use full
first names wherever possible. Include authors\emoji{’} e-mail addresses
whenever possible.

Grouping authors\emoji{’} names or e-mail addresses, or providing an
...
note to apply to multiple authors \emoji{—} for example, if the first
two authors of an article contributed equally to the work.
...
of the authors\emoji{’} names, which may result in overlapping text in
the page headers.

The article template\emoji{’}s documentation, available at
\url{https://www.acm.org/publications/proceedings-template}, has a
complete explanation of these commands and tips for their effective use.

Note that authors\emoji{’} addresses are mandatory for journal articles.
...
The ACM Computing Classification System \emoji{—}
\url{https://www.acm.org/publications/class-2012} \emoji{—} is a set of
...
the authors\emoji{’} choosing, providing a more flexible way of
...
``\texttt{booktabs}'' package \emoji{—}
\url{https://ctan.org/pkg/booktabs} \emoji{—} for preparing high-quality
tables.
...
\textbf{table} to enclose the table\emoji{’}s contents and the table
caption. The contents of the table itself must go in the
\textbf{tabular} environment, to be aligned properly in rows and
columns, with the desired horizontal and vertical rules. Again, detailed
instructions on \textbf{tabular} material are found in the
\emph{\LaTeX User\emoji{’}s Guide}.
...
\begin{table}[htbp]
\centering
\caption{Frequency of Special Characters}\label{tbl-freq}
\begin{supertabular}{|>{\centering\arraybackslash}p{0.29\columnwidth}|>{\centering\arraybackslash}p{0.29\columnwidth}|p{0.29\columnwidth}|}
\hline
{\bfseries Non-English or Math} & {\bfseries Frequency} & {\bfseries Comments} \\
\hline
...
To set a wider table, which takes up the whole width of the
page\emoji{’}s live area, use the environment \textbf{table*} to enclose
the table\emoji{’}s contents and the table caption. As with a
single-column table, this wide table will ``float'' to a location deemed
more desirable. Immediately following this sentence is the point at
which Table~\ref{tbl-commands} is included in the input file; again, it
is instructive to compare the placement of the table here with the table
in the printed output of this document.

\begin{table}[htbp]
\centering
\caption{Some Typical Commands}\label{tbl-commands}
\begin{supertabular}{|>{\centering\arraybackslash}p{0.29\columnwidth}|>{\centering\arraybackslash}p{0.29\columnwidth}|p{0.29\columnwidth}|}
\hline
{\bfseries Command} & {\bfseries A Number} & {\bfseries Comments} \\
\hline
\texttt{\textbackslash{}author} & 100 & Author aaaaaaaaa aaaaaaaaaaaaaaaa-aaaaaaa aaaaaaaaaaaaaa-aaaaaaaaaa \\
\hline
...
i t only for this purpose. This enables assistive technologies to
...
\LaTeX{[}\textbf{Lamport:LaTeX?}{]}; this section will simply show a few
...
A numbered display equation\emoji{—}one set off by vertical space from
the text and centered horizontally\emoji{—}is produced by the
...
\textbf{displaymath} environment. Now, we\emoji{’}ll enter an unnumbered
equation:
...
just to demonstrate \LaTeX\emoji{’}s able handling of numbering.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{sample-franklin.png}}
...
\{\#fig-dresses fig-alt=``A woman and a girl in white dresses sit in an
open car.''\}
...
decorative. These descriptions convey what\emoji{’}s in the image to
someone who cannot see it. They are also used by search engine crawlers
for indexing images, and when images cannot be loaded.

A figure description must be unformatted plain text less than 2000
characters long (including spaces). \textbf{Figure descriptions should
not repeat the figure caption \emoji{–} their purpose is to capture
important information that is not already provided in the caption or the
main text of the paper.} For figures that convey important and complex
new information, a short text description may not be adequate. More
complex alternative descriptions can be placed in an appendix and
referenced in a short figure description. For example, provide a data
table capturing the information in a bar chart, or a structured list
representing a graph. For additional information regarding how best to
write figure descriptions and why doing this is so important, please see
...
The use of \BibTeX~for the preparation and formatting of one\emoji{’}s
references is strongly recommended. Authors\emoji{’} names should be
complete \emoji{—} use full first names (``Donald E. Knuth'') not
initials (``D. E. Knuth'') \emoji{—} and the salient identifying
features of a reference should be included: title, year, volume, number,
pages, article DOI, etc.
...
Some examples. A paginated journal article {[}\textbf{Abril07?}{]}, an
enumerated journal article {[}\textbf{Cohen07?}{]}, a reference to an
entire issue {[}\textbf{JCohen96?}{]}, a monograph (whole book)
{[}\textbf{Kosiur01?}{]}, a monograph/whole book in a series (see 2a in
spec. document) {[}\textbf{Harel79?}{]}, a divisible-book such as an
anthology or compilation {[}\textbf{Editor00?}{]} followed by the same
example, however we only output the series if the volume number is given
{[}\textbf{Editor00a?}{]} (so Editor00a\emoji{’}s series should NOT be
present since it has no vol.~no.), a chapter in a divisible book
{[}\textbf{Spector90?}{]}, a chapter in a divisible book in a series
{[}\textbf{Douglass98?}{]}, a multi-volume work as book
{[}\textbf{Knuth97?}{]}, a couple of articles in a proceedings (of a
conference, symposium, workshop for example) (paginated proceedings
article) {[}\textbf{Hagerup1993?}{]}, a proceedings article with all
possible elements {[}\textbf{Smith10?}{]}, an example of an enumerated
proceedings article {[}\textbf{VanGundy07?}{]}, an informally published
work {[}\textbf{Harel78?}{]}, a couple of preprints
{[}\textbf{AnzarootPBM14?}{]}, a doctoral dissertation
{[}\textbf{Clarkson85?}{]}, a master\emoji{’}s thesis:
{[}\textbf{anisi03?}{]}, an online document / world wide web resource
{[}\textbf{Poker06?}{]}, a video game (Case 1) {[}\textbf{Obama08?}{]}
and (Case 2){[}\textbf{Novak03?}{]} and {[}\textbf{Lee05?}{]} and (Case
3) a patent {[}\textbf{JoeScientist001?}{]}, work accepted for
publication {[}\textbf{rous08?}{]}, `YYYYb'-test for prolific author
{[}\textbf{SaeediMEJ10?}{]} and {[}\textbf{SaeediJETC10?}{]}. Other
cites might contain `duplicate' DOI and URLs (some SIAM articles)
{[}\textbf{Kirschmer?}{]}. Boris / Barbara Beeton: multi-volume works as
books {[}\textbf{MR781536?}{]} and {[}\textbf{MR781537?}{]}. A couple of
citations with DOIs: {[}\textbf{Kirschmer-2010-AEI-1958016-1958018?}{]}.
Online citations: {[}\textbf{CTANacmart?}{]}. Artifacts:
{[}\textbf{R?}{]} and {[}\textbf{UMassCitations?}{]}.

\section{Diagrams}\label{diagrams}

\includegraphics[width=\linewidth,height=0.8\textheight,keepaspectratio]{acm_files/mediabag/diagram-1.png}
...
My ref: {[}1{]}, {[}\textbf{holmeTemporalNetworks2012?}{]}
...
\bibitem[\citeproctext]{ref-D5D4TIHE}
\CSLLeftMargin{{[}1{]} }%
\CSLRightInline{Petter Holme and Jari Saramäki. 2012. Temporal networks.
\emph{Physics Reports} 519, 3: 97--125.
